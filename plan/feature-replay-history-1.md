...
| TASK-005 | Add integration test `Tests/ReplayHistoryEndpointTests.cs`: verify pagination metadata and that an identified caller receives the complete history after the route slug is normalized to its canonical `boardId`. (The membership-`403` gate and HiddenRanges filtering tests live in [feature-history-access-moderation-1.md](./feature-history-access-moderation-1.md).) | | |
