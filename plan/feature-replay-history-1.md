...
- **REQ-009**: Each history page is cached differently depending on whether it is complete or still growing, so that **complete pages are never re-requested from the server** (no revalidation poll): (a) **complete pages** (`pageNumber < totalPages` — full and immutable, since the append-only log never rewrites earlier pages) are served with `Cache-Control: public, max-age=31536000, immutable`, so browsers and shared proxies serve them straight from cache with **no conditional request** for their entire freshness lifetime; (b) the **final (growing) page** (`pageNumber == totalPages`) is served with `Cache-Control: no-cache` plus a `Last-Modified` header set to the timestamp of its most recent event, so clients always revalidate it via `If-Modified-Since` and get `304 Not Modified` while no new events have been appended. Each page is cached independently
...
| TASK-010 | Set per-page cache headers based on completeness (REQ-009). For a **complete page** (`pageNumber < totalPages`, full and immutable): emit `Cache-Control: public, max-age=31536000, immutable` and **no** `Last-Modified` is needed — caches serve it without ever polling the server. For the **final (growing) page** (`pageNumber == totalPages`): emit `Cache-Control: no-cache` and a `Last-Modified` header set to the timestamp of the page's most recent event, forcing revalidation. Distinguishing the two requires computing `totalPages` (already available from TASK-009's counts). | | |
| TASK-011 | Implement conditional GET handling for the final page via the `If-Modified-Since` request header (complete pages are `immutable` and never revalidate, so this only applies to `pageNumber == totalPages`): compare the request header timestamp with the page's `Last-Modified` timestamp (the most recent event on that page). If unchanged, return `304 Not Modified` (empty body, cached representation reused). Otherwise return `200` with full page content. This lets clients revalidate only the one mutable page while no new events exist on it. | | |
...
| TASK-014 | Add integration test `Tests/ReplayHistoryEndpointTests.cs`: verify pagination metadata (`totalEvents`, `totalPages`), oldest-first ordering, that a **complete page** carries `Cache-Control: public, max-age=31536000, immutable` (and is not revalidated), that the **final page** carries `Cache-Control: no-cache` with a `Last-Modified` equal to its most recent stroke and that a conditional GET with `If-Modified-Since` on the unchanged final page returns `304`, that requesting a non-existent page returns `404`, and that all pages combined contain the complete unfiltered history. (The membership-`403` gate and HiddenRanges filtering tests live in [feature-history-access-moderation-1.md](./feature-history-access-moderation-1.md).) | | |
...
- **TEST-002**: Integration test — a complete page (`pageNumber < totalPages`) carries `Cache-Control: public, max-age=31536000, immutable`; the final page carries `Cache-Control: no-cache` with `Last-Modified` equal to its most recent stroke; requesting a non-existent page returns `404`; all pages combined contain the complete unfiltered history
- **TEST-003**: Integration test — a conditional GET with `If-Modified-Since` on the unchanged **final** page returns `304 Not Modified`
