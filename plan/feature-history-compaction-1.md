...
- **REQ-016**: Keyframe invalidation MUST be **atomic with — or strictly ordered after** — the owner action that triggers it and safe under concurrency (REQ-012), leaving no window in which the action has taken effect but a stale keyframe is still served. Until a rebuilt keyframe is available, history/replay load falls back to folding the event log (with the board's current HiddenRanges applied) or reading the authoritative `Board.ActiveStrokes` snapshot, so correctness never depends on a present or fresh keyframe (consistent with REQ-006)
