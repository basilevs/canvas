...
| TASK-043 | Create `wwwroot/js/connection.js` — SignalR connection module: establish connection to `/hub/whiteboard` (identity via cookie, no client-supplied userId), implement `JoinBoard`, handle `LoadSnapshot`/`StrokeReceived`/`UserJoined`/`UserLeft`/`UserRenamed` events. Note: there is no `StrokeRemoved` handler — undo is not part of the MVP (it is introduced by [feature-replay-history-1.md](./feature-replay-history-1.md)); administration events `UserRemoved`/`BoardSettingsChanged`/`Kicked`/`AccessDenied`/`InvalidInvite` and `joinBoardWithInvite` per [feature-board-administration-1.md](./feature-board-administration-1.md); visibility moderation events `StrokesHidden`/`StrokesRestored` per [feature-visibility-moderation-1.md](./feature-visibility-moderation-1.md) | | |
