...
- **REQ-016**: Stroke submission is idempotent — each stroke carries a client-generated GUID `Id`, and the server de-duplicates by that id so a stroke re-sent after a SignalR reconnect/retry is stored and broadcast at most once
...
- **SEC-004**: The app emits hardening response headers on every response — a Content-Security-Policy (`default-src 'self'`; `script-src 'self'`; `style-src 'self'` plus the Pico.css CDN host; `connect-src 'self'` so the SignalR WebSocket connects same-origin; `img-src 'self' data:`; `frame-ancestors 'none'`) plus `X-Content-Type-Options: nosniff` — so that even if an attacker found an injection point, no inline script runs and the cookie-theft path (RISK-004) is contained, and the page cannot be framed for clickjacking. All app JavaScript is loaded from same-origin `wwwroot/js/` files (no inline `<script>`), which the CSP requires
- **SEC-005**: The app is defended against CSRF **without** per-request antiforgery tokens because identity is an *ambient* cookie: the `uid` cookie is `SameSite=Strict` (TASK-006), so a cross-site page cannot make the browser attach the identity cookie to any request. Every state-changing operation is either a SignalR hub call or a non-`GET` REST method — there are **no side-effecting `GET` endpoints** (the snapshot `GET` is read-only and never creates a board; creation happens only in the authenticated hub `JoinBoard`). As defense-in-depth the app configures **no cross-origin CORS policy** and the SignalR hub validates the request `Origin`, closing the cross-site WebSocket hijacking vector (RISK-006)
...
- **CON-006**: Production is served over HTTPS only and never accepts plaintext HTTP (TLS terminated at the edge). Therefore the app adds **neither** `UseHttpsRedirection` (there is no HTTP listener to redirect from) **nor** `UseHsts` (HSTS is redundant when HTTP is never served). The `Secure` cookie flag in `UserIdentityMiddleware` is gated on `context.Request.IsHttps` purely so identity still works over `http://localhost` in development — in production every request is HTTPS, so the flag is always set
...
- **GUD-003**: REST endpoints must never expose persistence/domain models (`Board`, `Stroke`, `StrokeEvent`) directly; define `sealed record` response DTOs (e.g. `BoardSnapshotResponse`, `StrokeResponse`) with `<summary>` XML doc comments, and use UTC `DateTime` (not `DateTimeOffset`) for any date/time fields — all timestamps are UTC server-assigned, so no timezone/offset is carried anywhere
...
- **GUD-010**: Treat the identity token as a secret in logs — never log the `uid` cookie value or `HttpContext.Items["UserId"]`. If HTTP request/response logging is enabled, configure it to redact the `uid` cookie and the `Cookie`/`Set-Cookie` headers (e.g. exclude them from `AddHttpLogging` fields) so the session token cannot leak into log sinks
- **GUD-011**: Do not enable a permissive CORS policy (`AllowAnyOrigin`, or any cross-origin policy combined with `AllowCredentials`). The SPA, REST API, and SignalR hub are all served same-origin, so no CORS configuration is required; adding a credentialed cross-origin policy would undermine the `SameSite=Strict` CSRF protection (SEC-005)
...
| TASK-005 | Add `builder.Services.AddSignalR()` and map hub endpoint `app.MapHub<WhiteboardHub>("/hub/whiteboard")` in `Program.cs`. Do **not** configure any cross-origin CORS policy (the frontend is served same-origin per GUD-011); restrict the hub to same-origin by validating the request `Origin` on connection and rejecting cross-origin WebSocket handshakes — defense-in-depth against cross-site WebSocket hijacking (CSWSH, RISK-006) on top of the `SameSite=Strict` identity cookie. | | |
| TASK-006 | Create identity middleware in `Middleware/UserIdentityMiddleware.cs`: on each request, check for a `uid` HttpOnly cookie. If absent, generate a new UUID, set it as an HttpOnly, `SameSite=Strict`, **host-only** (do not set a `Domain` attribute, so sibling subdomains cannot plant the cookie) cookie, and attach to `HttpContext.Items["UserId"]`. Set the `Secure` flag only when the request is HTTPS (`context.Request.IsHttps`) so identity still works over `http://localhost` in development — a `Secure` cookie is never returned over plain HTTP (in production, which is HTTPS-only per CON-006, the flag is always set). In the same middleware, emit the SEC-004 hardening response headers on every response (`Content-Security-Policy` allowing only same-origin plus the Pico.css CDN host for styles and `connect-src 'self'` for the SignalR WebSocket, `frame-ancestors 'none'`, and `X-Content-Type-Options: nosniff`). SignalR hub reads userId from `Context.GetHttpContext().Items["UserId"]` — client never supplies its own identity. | | |
...
| TASK-015 | Create `Models/Stroke.cs` — properties: `Id` (string — a **client-generated GUID** supplied with the stroke; the idempotency / de-duplication key and the stable id used by undo and replay), `BoardId` (string), `UserId` (string, server-assigned from the connection identity), `Points` (List<Point>), `Color` (string), `Width` (float), `Timestamp` (UTC `DateTime`, server-assigned when the stroke was received), `Duration` (long, milliseconds from first point to last point in the stroke as measured by client). Note: the per-event ordering field `SequenceNumber` is not on the snapshot stroke — it is an event-log concern owned by `StrokeEvent` in [feature-replay-history-1.md](./feature-replay-history-1.md) (the MVP snapshot preserves order implicitly via the `$push` append order of `ActiveStrokes`). | | |
...
| TASK-026 | Implement `SendStroke`: validate the inbound `StrokeInput` (including that `Id` is a well-formed GUID), build a `Stroke` carrying the client-supplied `Id` with server-assigned `UserId`/`Timestamp`, append it via `AddStrokeToSnapshotAsync(boardId, stroke)` (idempotent atomic `$push` guarded so a stroke whose `Id` is already in `ActiveStrokes` is not appended again — handles SignalR reconnect re-sends; uses the `boardId` cached in the connection map, no per-stroke board lookup), and broadcast `StrokeReceived(stroke)` **only when the stroke was newly appended**, so duplicates are neither stored nor re-broadcast. Note: member-access enforcement (verify the caller is a member, reject non-members with `AccessDenied`) is layered onto this method by [feature-board-administration-1.md](./feature-board-administration-1.md), and append-only event persistence (write an `Add` StrokeEvent) is layered on by [feature-replay-history-1.md](./feature-replay-history-1.md); the MVP has no membership concept and persists only the snapshot, so any connected user may draw. | | |
...
| TASK-042 | Create `wwwroot/js/canvas.js` — Canvas drawing module: capture mousedown/mousemove/mouseup and touch events, assign each stroke a client-generated GUID `id` via `crypto.randomUUID()` at stroke start (the idempotency key sent with the stroke and reused unchanged on any reconnect retry), collect points into stroke objects with `TimeOffset` (ms since stroke start via `performance.now()`), compute `Duration`, render strokes on canvas using `CanvasRenderingContext2D` path API; also render a lightweight remote-cursor overlay (one marker per userId, updated on `CursorMoved`, removed on `UserLeft`) drawn above the strokes | | |
...
| TASK-045 | Create `wwwroot/js/app.js` — Application orchestrator: route join method, wire display name input to `SetDisplayName` on blur/enter, manage board state, handle reconnection (on reconnect, re-send any strokes not yet confirmed using their original client-generated `id` so the server de-duplicates them), maintain the connected-users list on `UserJoined`/`UserLeft`/`UserRenamed`. Note: invite-URL detection and `Kicked` handling per [feature-board-administration-1.md](./feature-board-administration-1.md) | | |
...
| Task | Description | Completed | Date |
|------|-------------|-----------|------|
| TASK-054 | Create response DTOs as `sealed record` types in `Dtos/` (GUD-003), each with `<summary>` XML doc comments and UTC `DateTime` for date/time fields: `BoardSnapshotResponse` (board name, `IReadOnlyList<StrokeResponse>`), `StrokeResponse` (id, userId, color, width, `IReadOnlyList<PointResponse>`, `Timestamp`), `PointResponse` (x, y, pressure, timeOffset), `ConnectedUserResponse` (userId, displayName). Also define inbound (client→server) hub DTOs: `StrokeInput` (a client-generated GUID `Id` (idempotency key), color, width, duration, `IReadOnlyList<PointInput>`) and `PointInput` (x, y, pressure, timeOffset) — the server assigns `UserId`/`Timestamp` but accepts the client's random `Id` (validated as a well-formed GUID). Map domain models → DTOs (and `StrokeInput` → `Stroke`) in the service/endpoint/hub layer; never serialize `Board`/`Stroke`/`StrokeEvent` directly. Date/time fields pass through unchanged (UTC `DateTime` in both model and DTO). Note: the board-listing DTO `BoardSummaryResponse` is defined by [feature-board-management-1.md](./feature-board-management-1.md). | | |
| TASK-037 | Add minimal-API endpoint `GET /api/boards/{name}/snapshot` — returns `200 OK` with `BoardSnapshotResponse`, or `404 Not Found` if the board does not exist; open access in the MVP (all boards public). This `GET` is strictly read-only and side-effect-free — it never creates the board (creation happens only in the authenticated hub `JoinBoard` via `GetOrCreateBoardAsync`), so it is safe to expose as a `GET` and carries no CSRF/prefetch risk (SEC-005). Accept `CancellationToken`; use `TypedResults` with an explicit `Results<Ok<BoardSnapshotResponse>, NotFound>` return type; chain `.WithName("GetBoardSnapshot").WithSummary(...).Produces<BoardSnapshotResponse>(200).Produces(404)`. Note: private-board access enforcement (require userId + membership check, return `403`) is layered onto this endpoint by [feature-board-administration-1.md](./feature-board-administration-1.md). | | |
...
| TASK-051 | Add MSTest integration test (GUD-008): `Tests/WhiteboardHubTests.cs` — verify JoinBoard returns the snapshot and broadcasts `UserJoined`, SendStroke adds the stroke to the board snapshot and broadcasts `StrokeReceived`, and re-sending a stroke with an already-seen `Id` is a no-op (the snapshot contains it exactly once and `StrokeReceived` is broadcast once) — idempotency. (Private-board access-denial tests are in [feature-board-administration-1.md](./feature-board-administration-1.md); undo and event-log tests are in [feature-replay-history-1.md](./feature-replay-history-1.md).) | | |
...
- **FILE-034**: `Dtos/` — `sealed record` response DTOs (`BoardSnapshotResponse`, `StrokeResponse`, `PointResponse`) with XML doc comments and UTC `DateTime` date/time fields (`BoardSummaryResponse` is defined by [feature-board-management-1.md](./feature-board-management-1.md))
...
- **TEST-021**: Integration test — Re-sending a stroke with an already-seen `Id` (e.g. after a reconnect) is a no-op: the snapshot contains the stroke exactly once and `StrokeReceived` is broadcast only on first receipt
...
- **RISK-004**: If the HttpOnly cookie is stolen (e.g., via XSS on a related domain), an attacker could impersonate the user — mitigated by `HttpOnly` (script cannot read the cookie), `SameSite=Strict`, the `Secure` flag, and a strict Content-Security-Policy with no inline scripts (SEC-004)
- **RISK-005**: The identity middleware trusts any well-formed `uid` cookie it receives, so a pre-planted cookie could fix a session (session fixation). Planting the cookie cross-site is already prevented by issuing it **host-only** (no `Domain` attribute, so sibling subdomains cannot set it), `SameSite=Strict`, `HttpOnly`, and the strict CSP (SEC-004) — leaving no practical injection vector in the HTTPS-only deployment (CON-006). The residual risk is benign for the anonymous, throwaway MVP because no privilege attaches to an identity; when [feature-board-administration-1.md](./feature-board-administration-1.md) makes the first accessor a board owner it should, as defense-in-depth, reissue (rotate) the `uid` on the next HTTP response after ownership is established
- **RISK-006**: Cross-site WebSocket hijacking (CSWSH) — WebSocket handshakes are not governed by the same-origin policy/CORS, so a malicious page could attempt to open a connection to `/hub/whiteboard` riding the victim's ambient cookie. Mitigated primarily by the `SameSite=Strict` `uid` cookie (a cross-site handshake carries no identity cookie, so the connection only ever receives a fresh anonymous identity, never the victim's) and, as defense-in-depth, by the hub rejecting connections whose `Origin` is not same-origin and the absence of any cross-origin CORS policy (TASK-005, GUD-011)
