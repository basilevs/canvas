...
| TASK-015 | Create `Models/Stroke.cs` — properties: `Id` (ObjectId), `BoardId` (string), `UserId` (string), `Points` (List<Point>), `Color` (string), `Width` (float), `Timestamp` (UTC `DateTime`, server-assigned when the stroke was received), `Duration` (long, milliseconds from first point to last point in the stroke as measured by client). Note: the per-event ordering field `SequenceNumber` is not on the snapshot stroke — it is an event-log concern owned by `StrokeEvent` in [feature-replay-history-1.md](./feature-replay-history-1.md) (the MVP snapshot preserves order implicitly via the `$push` append order of `ActiveStrokes`). | | |
