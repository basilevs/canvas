...
- **REQ-004**: Snapshot and history endpoints are filtered by the owner's visibility settings (HiddenRanges) for non-owner members; only the owner can access the unfiltered view
...
- [Replay & History View feature plan](./feature-replay-history-1.md) — replay history is filtered using the same HiddenRanges
