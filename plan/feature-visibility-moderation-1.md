...
| Task | Description | Completed | Date |
|------|-------------|-----------|------|
| TASK-008 | Add hub methods to `Hubs/WhiteboardHub.cs`: `HideContributions(boardName, targetUserId)`, `RestoreContributions(boardName, targetUserId)`, `ToggleShowHidden(boardName, showHidden)` — all normalize the route slug to its canonical `boardId` (GUD-012) and resolve userId from `Context.GetHttpContext().Items["UserId"]`. Per the parent plan's strongly typed hub convention (GUD-009), extend `IWhiteboardClient` with the server→client events this plan introduces (`StrokesHidden`, `StrokesRestored`) and broadcast them via the typed proxy (e.g. `Clients.Group(name).StrokesHidden(...)`), never `SendAsync`. | | |
...
| TASK-012 | Ensure `JoinBoard` snapshot delivery normalizes the route slug to its canonical `boardId` once (GUD-012) and uses `GetSnapshotAsync(boardId, callerUserId)` so newly joining non-owner members receive the filtered snapshot, and owners receive the full snapshot. | | |
