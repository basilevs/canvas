...
| TASK-015 | In `wwwroot/index.html`/`app.js`, add the invite-token landing affordance: the invite link is the board's editing URL carrying the token as a credential — `/boards/{name}?invite={token}` (it lives in the editing namespace, GUD-013, not a separate top-level action path). The SPA — served for `/boards/{name}` by the scoped fallback (parent TASK-011) — detects the `?invite={token}` query parameter and redeems it over the hub via `joinBoardWithInvite` (TASK-017). Redemption is therefore a **non-`GET` hub action**, so no side-effecting `GET` is introduced and the parent plan's CSRF stance (SEC-005) is preserved. Cookie note: an invite link is typically opened as a **cross-site top-level navigation** (from chat/email), and the parent plan's `SameSite=Strict` `uid` cookie is withheld on that first cross-site navigation — so the landing would mint a fresh identity instead of reusing the visitor's existing one. When invites are introduced, switch the identity cookie to `SameSite=Lax` (still CSRF-safe — it blocks cross-site sub-requests and cross-site POSTs and permits only top-level `GET` navigations, while all state-changing operations remain SignalR/non-`GET`), or bounce the invite through a same-site redirect so the cookie is present on the subsequent same-site request. | | |
