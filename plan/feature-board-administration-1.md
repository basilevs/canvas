...
| TASK-007 | Implement `CreateInvite(boardName)`: verify caller is owner OR (`MembersCanInvite` is true AND caller is member). Call `InviteService.CreateInviteAsync`, return the invite URL to the caller in the form `/boards/{boardName}?invite={token}` (the editing URL carrying the token as a credential, GUD-013). | | |
...
| TASK-011 | **Extend the MVP's existing connection tracking** (parent TASK-039, `ConcurrentDictionary<connectionId, UserConnection>`) with a `userId → set of connectionIds` **reverse index**, maintained in the *same* `OnConnectedAsync`/`OnDisconnectedAsync` paths as the primary map (not a second, independently-managed structure), so `RemoveMember` can locate and disconnect all of a target user's connections. Keep both views consistent by updating them together under the same add/remove operations. | | |
...
| TASK-015 | In `wwwroot/index.html`/`app.js`, add the invite-token landing affordance: the invite link is the board's editing URL carrying the token as a credential — `/boards/{name}?invite={token}` (it lives in the editing namespace, GUD-013, not a separate top-level action path). The SPA — served for `/boards/{name}` by the scoped fallback (parent TASK-011) — detects the `?invite={token}` query parameter and redeems it over the hub via `joinBoardWithInvite` (TASK-017). Redemption is therefore a **non-`GET` hub action**, so no side-effecting `GET` is introduced and the parent plan's CSRF stance (SEC-005) is preserved. No invite "landing"/redirect endpoint is needed: the redeeming `joinBoardWithInvite` call runs over the SignalR connection the loaded SPA opens to our own origin, which is a **same-site** subrequest, so the page's `uid` cookie attaches to it regardless of how the user first arrived. Note the `SameSite=Strict` consequence for invitees who follow the link from another origin (e.g. email/chat): the existing `uid` cookie is withheld on that cross-site top-level `GET`, so the identity middleware (parent TASK-006) issues a **fresh** `uid` and the invitee joins as a new anonymous identity — this is acceptable for the anonymous, throwaway identity model (no durable account or privilege is lost) and the freshly issued identity is the one that redeems the invite over the same-site hub call. | | |
...
- **DEP-006**: `wwwroot/js/admin.js`, `connection.js`, `app.js`, `index.html` — owner panel and invite landing affordance (`?invite={token}` query detection on the `/boards/{name}` editing URL)
...
- **FILE-008**: `wwwroot/js/app.js` — Invite query-param (`?invite={token}`) detection and kick handling
- **FILE-009**: `wwwroot/index.html` — Invite token landing affordance (on the `/boards/{name}` editing page)
