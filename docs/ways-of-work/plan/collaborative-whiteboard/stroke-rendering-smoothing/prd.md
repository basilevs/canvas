...
- **R1 — Tessellated per-segment polyline (recommended default).** Evaluate the §9 curve at many short sub-segments, then for each sub-segment `beginPath` → `moveTo`/`lineTo` → set `lineWidth` from the interpolated pressure → `stroke()`. This is the current model (one `stroke()` per pair) with more, curve-following segments. *Pros:* variable width works at sub-segment granularity; minimal change from today; trivially supports `upToMs` (stop emitting at the cutoff). *Cons:* many `stroke()` calls per stroke (the main cost; mitigate by choosing tessellation resolution per NFR-1 and/or caching the sub-segment list). *Best when:* you want pressure width with the smallest, safest change. *Best algorithm pairing:* **Option B (centripetal Catmull-Rom)** — it interpolates the true anchors and tessellates cleanly; Option A also works if corner-rounding is acceptable.
- **R2 — Native curve calls, single stroke (`quadraticCurveTo`/`bezierCurveTo`).** Build one `Path2D`/path: `moveTo` the first point, then one `quadraticCurveTo`/`bezierCurveTo` per span (control points from §9), then a single `stroke()`. *Pros:* fewest API calls, best-quality native antialiasing of the curve and joins, cheapest per redraw; a `Path2D` can be **built once per stroke and cached**, then re-stroked on every redraw (big NFR-1 win). *Cons:* one `lineWidth` for the whole path — **cannot vary thickness along the curve**, so it only fits constant-width strokes (mouse / `null`-pressure, or if pressure width is dropped). *Best when:* the stroke has uniform width; pair with R1/R3 for pressure strokes. *Best algorithm pairing:* **Option A (quadratic Bézier through midpoints)** — it maps one-to-one onto `quadraticCurveTo` (stored point = control point, midpoints = on-curve joins) with no per-span control-point derivation and no overshoot; **Option B** is the runner-up via one `bezierCurveTo` per span when interpolating the actual points matters.
- **R3 — Per-span native curves, broken path (hybrid).** Like R2 but call `stroke()` once per span (or per few spans) with that span's averaged width, instead of one stroke for the whole path. *Pros:* native curve quality with width that varies at span granularity — far fewer `stroke()` calls than R1 for similar smoothness; *Cons:* width steps per span rather than per sub-segment (coarser than R1, though §9 pressure smoothing softens this); slightly more bookkeeping. *Best when:* you want native-curve smoothness *and* pressure width with fewer draw calls than R1. *Best algorithm pairing:* **Option B (cubic `bezierCurveTo` per span)**, or **Option A (`quadraticCurveTo` per span)** for the simplest control points.
- **R4 — Filled variable-width ribbon (outline fill).** Don't stroke a centerline at all: offset the §9 curve by ±½·width(pressure) along its normal to build left/right boundary curves, close them into one `Path2D`, and `fill()` once (round caps via end arcs; use `'nonzero'` fill to absorb self-overlap on tight turns). *Pros:* true, continuously-varying width in a single `fill()` — the highest-quality "brush" look, closest to real ink; one cached `Path2D` per stroke. *Cons:* most math (normal/offset computation, self-intersection handling on sharp turns); harder to get right; `upToMs` clipping means rebuilding the partial outline per frame. *Best when:* width fidelity/aesthetics matter most and you can absorb the implementation cost.

**Recommendation for Option R:** default to **R1** (tessellated per-segment) — it preserves today's pressure-width behavior with the smallest change — and use **R2** as a fast path for constant-width (`null`-pressure) strokes via a cached `Path2D`. Consider **R3** if per-redraw draw-call count becomes the bottleneck, and **R4** only if brush-quality variable width becomes a goal. Across all variants, build and **cache a per-stroke path/tessellation** and reuse it across redraws so confirmed-history repaints stay within the NFR-1 budget.

> **First explorative spike — Option A + R2.** Before committing to the recommended R1/Option B default, prototype **Option A (quadratic Bézier through midpoints) rendered via R2 (single `Path2D`, one `stroke()`)** as the initial experiment. Rationale: it is the **easiest to implement** (one `quadraticCurveTo` per point, no tangent/control-point math, no tessellation) and the **fastest on mobile** (one cached native path stroked per redraw, fewest draw calls), making it the cheapest way to **evaluate how visible the approximation errors actually are during natural drawing**. If the midpoint approximation's corner-rounding proves acceptable in real use, it may even stand as the shipped approach; if not, the evaluation directly informs whether to move to interpolating Option B and/or pressure-width variants (R1/R3). Constant-width during the spike is acceptable since its purpose is to measure geometric error, not pressure fidelity.
