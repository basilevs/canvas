...
2. **Capture-time (evaluated alternative/complement):** enrich the captured data so the smooth curve is reconstructable from it — **without moving the exact anchor points**. This means recording compact per-point direction/tangent hints, or changing the format to store curve parameters (Bézier controls) instead of a raw polyline. In-place smoothing/resampling that overwrites point positions is rejected as lossy at the anchors. These options can improve fidelity under aggressive simplification but change stored/synced data and must be weighed against that cost.
...
- **FR-8 — Placement decision is explicit and documented.** The chosen smoothing placement — render-time (inside `drawStrokePath`) versus capture-time (enriching captured data with per-point tangent hints, or storing curve parameters, while keeping exact anchors) — must be a deliberate, recorded decision per §10. For the **render-time** default, capture (`#appendPoint`/`#toCanvasPoint`), `simplifyPointsVisvalingamWhyatt`, the stroke/point DTOs, and hub messages remain untouched. If a **capture-time** approach is adopted, it must keep every captured anchor exact (no in-place resampling/smoothing of positions), justify the change against §10's trade-offs, preserve backward compatibility of the existing DTOs, and keep simplification and sync working.
...
- Predictive input / lag-hiding or point *prediction* (synthesising points the device never reported). Capture-time **enrichment that preserves exact anchors** (per-point tangent hints, or storing curve parameters) is in scope and evaluated in §10; *predicting* unseen points, or in-place resampling/smoothing that moves anchors, is not.
...
Capture-time approaches must **preserve the exact anchor points**. Smoothing or resampling the captured points in place (EMA/median/arc-length resampling that overwrites positions) is an **anti-pattern and explicitly rejected**: it moves the anchors, losing precision even at the points the user actually drew, and corrupts the data every downstream consumer relies on. The two viable capture-time options below instead keep every anchor exact and add the curvature information *alongside* it.

### Option C1 — Capture-time tangent hints (exact anchors + per-point direction)

- **How:** keep every stored anchor point exactly as captured, but record a compact, low-resolution **direction (tangent) hint** per point — the local incoming/outgoing heading, quantized to a few bits. At render time the curve is a Hermite/Bézier spline that passes **through** each anchor using the stored tangent, instead of inferring the tangent from neighbors (as render-time Catmull-Rom must).
- **Pros:** anchors are lossless; the curve reproduces the user's true direction at each point even after `simplifyPointsVisvalingamWhyatt` removes neighbors, so simplification can be more aggressive without changing the curve's shape; tangents are tiny (a quantized angle per point) so the storage/bandwidth cost is small; render-time work drops to evaluating a spline with known tangents (cheaper, no neighbor analysis).
- **Cons:** adds an optional per-point field to the point DTO (must be backward compatible: absent ⇒ fall back to render-time tangent inference); simplification must recompute/preserve the surviving points' tangents when it drops a neighbor; tangent must be captured/derived during `#appendPoint` from the local sample window.
...
- **How:** change the stored representation from a polyline of sampled points to **curve parameters** — e.g. a sequence of cubic Bézier segments (anchor + control points) fit to the captured input at capture time. Anchors remain exact knots of the fitted curve; the controls carry the smoothing.
...
- **Cons:** the largest format change — new stroke/point schema, DTO and wire changes, and a migration/back-compat path for existing polyline boards (renderer must handle both shapes); replay's point-by-point `upToMs` reveal must be redefined in terms of curve arc length/parameter rather than point index; fitting adds capture-time cost and must bound deviation (NFR-2) so anchors stay faithful.
- **Best when:** you want curves as the canonical data model and are willing to take a deliberate format/migration project to get cross-consumer consistency and maximal compression.

### Recommendation

Ship **Option R (render-time)** first — it delivers the visible smoothing win with no data-format risk and benefits all surfaces from a single, reversible change. If a capture-time fidelity gain is later justified, prefer **Option C1 (tangent hints)** as the incremental step: it keeps anchors exact, is backward compatible (tangent optional), and mainly helps strokes survive aggressive simplification without shape change. Reserve **Option C2 (curve parameters)** for a deliberate, separately-scoped format change when curves-as-data is a goal in its own right. Do **not** adopt the in-place resample/smooth approach — it is lossy at the anchors and rejected above. Whichever path is chosen, the curve math (§9) and its tunable constants (FR-10) are shared so render-time and capture-time never diverge (NFR-7), and the curve must be applied **once** to avoid double-smoothing if approaches are combined.
